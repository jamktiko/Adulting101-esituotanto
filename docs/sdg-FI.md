...
Kaikilla on yhtä tärkeä vastuu noudattaa kestävää kehitystä. Seuraamalla ekologista kestävyyttä pyrimme hillitsemään päästöjä ja pitämään maapallon elinkelpoisena myös tuleville sukupolville. Sosiaalista kestävyyttä seuraamalla pidämme huolen tasa-arvon toteutumisesta. Taloudellista kestävyyttä seuraamalla pyrimme kiertotalouteen, joka säästää rahaa ja luonnonvaroja.
...
Sovellus edistää terveyttä ja hyvinvointia ja auttaa nuoria aikuisia itsenäiseen elämään, johon kuuluu olennaisena osana myös se, millaisilla elämänvalinnoilla saa ylläpidettyä kestävää kehitystä.
...
Sovelluksen kehitysprosessissa tekoälyä käytetään harkitusti. Sovelluksessa säilötään vain sovelluksen käytön kannalta olennaisia tietoja. Ihanteellisessa tilanteessa sovelluksessa olisi tekoälyrobotti, jolta kysyä apua, mitä sovelluksesta ei muutoin löydä. Tämän toteuttaminen voi tosin olla haastavaa.
...
Sovelluksen pääasiallinen tavoite ja tarkoitus on vähentää luonnonvarojen kulutusta. Tarkoitus on kannustaa sovelluksen käyttäjää järkeviin valintoihin. Sovellusta ei käytetä muuhun kulutukseen eikä se edellytä muiden laitteiden hankintaa.
...
Dokumentoimme kehitysprosessia projektin edetessä.
...
Sovelluksen peruskäyttö on ilmaista. Sovellus on saatavilla tässä vaiheessa vain suomeksi. Sovellus ei tee suoraan oletuksia käyttäjän ominaisuuksista, mutta sovelluksen pääasiallinen kohderyhmä on nuoret, itsenäistymisen kynnyksellä olevat aikuiset. Sovelluksen visuaaliset elementit on selkeytetty tekstillä.
...
Sovelluksen tavoite on lisätä hyvinvointia ja vaikutusten elämänhallintaan ja taloudelliseen tilanteeseen odotetaan olevan positiivisia. Sovelluksen käyttö kannustaa tekemään arkielämässä kestävää kehitystä tukevia valintoja.
...
Sovellusta ei voi syyttää muiden ihmisten häiriintymisestä. Sovellus ei tarjoa alustaa disinformaation tai vihapuheen jakamiselle. Sovellus ei kannusta käyttäjää rikolliseen toimintaan, eikä sovellusta siihen voi myös hyödyntää. Sovelluksen vaikutukset näkyvät pääasiassa yksilön hyvinvoinnissa ja elämässä.
...
Sovelluskehityksessä on jaettu selkeät vastuualueet ja kehittäjillä ei ole epäselvyyttä siitä, mikä heidän pääasiallinen tehtävänsä on. Kehittäjät pääsevät lounastauolle päivittäin. Kehittäjät eivät saa tästä penniäkään. Jos rahoitus olisi, se olisi askel eteenpäin turvalliseen ylläpitoon ja jatkokehitykseen. Noudatamme Scrumia. Tavoitteet ovat selkeitä.
